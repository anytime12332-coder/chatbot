...
RUN printf '#!/bin/sh\n\
cd /app/server\n\
echo "=== Step 1: Fast Database Schema Sync ==="\n\
npx prisma db push --accept-data-loss --skip-generate 2>&1 || echo "WARNING: DB sync warning"\n\
echo "=== Step 2: Database Seed ==="\n\
node src/seed.js 2>&1 || echo "WARNING: Seed warning"\n\
